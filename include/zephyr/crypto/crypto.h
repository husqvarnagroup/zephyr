 ...
 * @param  ctx       Pointer to the crypto context of this op.
 * @param  pkt   Structure holding the input/output buffer pointers.
 * @param  ctr[in,out] Counter for the operation. Based on IV used during session initialization,
 *                     this argument gets initialized on the first run, and updated after every
 *                     invocation. User needs to provide the same buffer during the whole session.
 *
 * @return 0 on success, negative errno code on fail.
 */
