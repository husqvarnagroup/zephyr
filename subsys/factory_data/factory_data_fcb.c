...
static int factory_data_init_fct(void)
{
	int ret;
...
static int factory_data_value_exists_callback(struct fcb_entry_ctx *const loc_ctx, void *const arg)
{
	const char *name = arg;
	uint8_t buf[CONFIG_FACTORY_DATA_NAME_LEN_MAX + 1];
	int ret;
	uint16_t max_read_len = MIN(loc_ctx->loc.fe_data_len, sizeof(buf));
	size_t name_len;

	ret = flash_area_read(loc_ctx->fap, FCB_ENTRY_FA_DATA_OFF(loc_ctx->loc), buf, max_read_len);
	if (ret) {
		return -EIO;
	}
...
static int factory_data_load_callback(struct fcb_entry_ctx *const entry, void *const arg)
{
	struct factory_data_load_callback_ctx *ctx = arg;
	uint8_t buf[FACTORY_DATA_TOTAL_LEN_MAX];
	int ret;
	uint16_t fcb_entry_len = entry->loc.fe_data_len;
	size_t name_len;
	size_t value_len;
	...
	ret = flash_area_read(entry->fap, FCB_ENTRY_FA_DATA_OFF(entry->loc), buf, fcb_entry_len);
	if (ret) {
		return -EIO;
	}
