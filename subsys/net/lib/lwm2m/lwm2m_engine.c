...
int lwm2m_send_message(struct lwm2m_message *msg)
{
	...
	rc = sendto(msg->ctx->sock_fd, msg->cpkt.data, msg->cpkt.offset, 0, &msg->ctx->remote_addr, NET_SOCKADDR_MAX_SIZE);
...
static void retransmit_request(struct k_work *work)
{
	...
	if (sendto(msg->ctx->sock_fd, msg->cpkt.data, msg->cpkt.offset, 0, &msg->ctx->remote_addr, NET_SOCKADDR_MAX_SIZE) < 0) {
	...
	while (1) {
		...
		for (i = 0; i < sock_nfds; i++) {
            ...
            if (sock_ctx[i]->remote_addr.sa_family == AF_INET) {
                ...
                from_addr_matches_remote_addr = net_ipv4_addr_cmp(remote_sin_addr, &net_sin(&from_addr)->sin_addr);
                ...
                from_addr_matches_remote_addr = net_ipv6_addr_cmp(remote_sin6_addr, &net_sin6(&from_addr)->sin6_addr);
            }
