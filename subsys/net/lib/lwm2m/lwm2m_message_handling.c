...
STATIC int build_msg_block_for_send(struct lwm2m_message *msg, uint16_t block_num)
{
	...
	if (is_raw_block_callback_in_use(msg)) {
		return build_msg_block_for_send_raw(msg, block_num);
	} else {
		return build_msg_block_for_send_default(msg, block_num);
	}
#else
	return build_msg_block_for_send_default(msg, block_num);
#endif
...
static int lwm2m_engine_send_impl(struct lwm2m_ctx *ctx, char const *path_list[], uint8_t path_list_size,
		      bool confirmation_request, lwm2m_engine_send_raw_cb_t raw_callback)
{
#if defined(CONFIG_LWM2M_SERVER_OBJECT_VERSION_1_1)
	struct lwm2m_message *msg;
	int ret;
...
#if defined(LWM2M_SUPPORT_RAW_BLOCK_TRANSFER)
int lwm2m_engine_send_raw_block_wise(struct lwm2m_ctx *ctx, char const *path,
		bool confirmation_request, lwm2m_engine_send_raw_cb_t callback)
{
	NET_ASSERT(callback != NULL, "A raw data callback is required");

	char const *path_list[1] = {path};
	return lwm2m_engine_send_impl(ctx, path_list, 1, confirmation_request, callback);
}
#endif

int lwm2m_engine_send(struct lwm2m_ctx *ctx, char const *path_list[], uint8_t path_list_size,
			   bool confirmation_request) {
	return lwm2m_engine_send_impl(ctx, path_list, path_list_size, confirmation_request, NULL);
}
